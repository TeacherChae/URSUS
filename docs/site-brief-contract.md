...
8. 위 mode별 legal-name field와 WFS `emd_kor_nm`은 `query-normalizer/1`과 동일하게 NFC 정규화·양끝 whitespace 제거·내부 whitespace collapse한 뒤 ordinal 비교한다. 다르면 성공을 유지하지 않고 `ProviderFailure`로 처리한다.
...
- generic provider reason에서 후보 2개를 허용하는 경우는 Road와 Parcel 주소 해석이 모두 성공한 뒤 `Boundary` WFS reported/schema/transport failure가 발생한 경우뿐이다. 주소 mode failure가 하나라도 있으면 성공 후보는 최대 1개다.
