...
- 빈 값 판정, provider request와 request cache에는 trim/collapsed query 값을 사용하지만 exact input provenance에는 사용자의 원문을 보존한다. `QueryFingerprint`는 semantic query 기준이므로 NFC/whitespace만 다른 입력은 같은 실행으로 본다. 원문 provenance만 달라진 경우 stale로 만들지 않되 새 실행 결과의 `InputAddress`에는 그 실행의 원문을 보존한다.
...
각 mode의 `OK`는 `status=OK`, finite이고 longitude `[-180,180]`·latitude `[-90,90]` 범위인 WGS84 point, non-empty `refined.text`와 일치하는 response contract를 모두 만족할 때만 인정한다.
...
| OK | OK, EPSG:5179 Euclidean 거리가 150m 이하이며 같은 법정동 | `Resolved`, `AddressKind=DualEquivalent`; 두 candidate를 모두 보존하고 Road를 display representative로 사용 |
| OK | OK, EPSG:5179 Euclidean 거리가 150m 초과 또는 법정동 불일치 | `NeedsSelection`; 자동 선택하지 않음 |
...
2. dual distance가 `150m` 이하이거나 single success라 경계 조회가 필요한 branch에서만 candidate의 `refined.structure.level4AC` 10자리 행정동 코드 앞 5자리를 시군구 bootstrap에 사용한다. 이 branch의 candidate는 `level1=서울특별시`, non-empty `level2`, 숫자 10자리 `level4AC`를 가져야 한다. 필드가 malformed면 해당 candidate를 성공 목록에서 제외하고 해당 mode의 `ProviderSchemaInvalid`로 변환한다. individually valid dual candidate의 시군구 prefix 또는 정규화한 `level2`가 다르면 `CohortBootstrapDisagreement / NeedsSelection`으로 두 candidate를 보존한다. 이 값은 법정동 ID를 선택하거나 corroboration하는 데 사용하지 않는다.
3. 같은 분석 요청에서 받은 해당 시군구 `lt_c_ademd_info` topology 각각에 대해 outer ring 내부이면서 hole 외부인지 even-odd ray-casting으로 판정한다.
4. 모든 predicate는 EPSG:5179에서 계산한다. point-to-segment projection parameter를 `[0,1]`로 clamp하고 zero-length segment는 endpoint distance를 쓴다. 모든 outer/hole segment 중 squared distance가 **`0.01m`의 제곱 이하**이면 edge match다. 이 tolerance는 deterministic topology 분류값일 뿐 측량·주소 정확도를 뜻하지 않는다.
5. candidate에 edge match가 하나라도 있으면 strict containment 결과와 무관하게 모든 edge alternative를 보존한 `NeedsSelection`이다. edge가 전혀 없는 topology만 half-open even-odd ray crossing에 참여한다. part는 outer 내부이면서 모든 hole 외부, MultiPolygon은 part union이고 같은 canonical 법정동의 part는 한 match로 합친다.
6. edge가 없고 정확히 한 topology가 strict containment이면 해당 WFS `emd_cd`, `full_nm`, `emd_kor_nm`을 선택한다.
7. edge가 없고 strict match가 0개면 `OutOfCoverage`, 2개 이상이면 `NeedsSelection`이다. 서로 다른 candidate 결과가 섞이면 `DistrictEdge → MultipleDistrictContainment → OutsideBoundaryCoverage` 순으로 먼저 선택하고, 같은 class를 둘 다 일으키면 Road를 representative로 사용한다. strict alternative의 `DistanceMeters`도 모든 outer/hole segment까지의 최소 거리다.
8. 모든 candidate가 unique strict일 때 위 mode별 legal-name field와 WFS `emd_kor_nm`을 `query-normalizer/1`과 동일하게 NFC 정규화·양끝 whitespace 제거·내부 whitespace collapse한 뒤 ordinal 비교한다. 다르면 성공을 유지하지 않고 `ProviderFailure`로 처리한다.
9. road/parcel candidate가 각각 선택한 canonical 법정동이 다르면 거리와 무관하게 `NeedsSelection`이다.

v1은 이미 검증된 cohort cache 계약을 유지하기 위해 위 strict `Level4AC` bootstrap을 사용한다. 전체 서울 467 boundary cache는 순환을 없애지만 Gate 2의 cohort별 exact-set/cache identity를 교체하므로 이번 단계에서는 도입하지 않는다. 선택된 canonical 법정동은 bootstrap cohort membership에 반드시 포함돼야 하며, 이를 위반하면 정상 coverage 결과가 아니라 Boundary `ProviderSchemaInvalid`다.
...
- `RepresentativeLocation`은 원인 `CandidateKind`, WGS84/EPSG:5179 point와 선택 근거를 가지며, 주소는 찾았지만 법정동을 확정하지 못한 경우에만 사용한다.
...
| `CohortBootstrapDisagreement` | `NeedsSelection` | 0 | 0 | 2 | 0 | 0 | 0 |
...
| `ProviderReportedError` | `ProviderFailure` | 0 | 0 | 0..2 | 0 | 0 | 1..2 |
| `ProviderSchemaInvalid` | `ProviderFailure` | 0 | 0 | 0..2 | 0 | 0 | 1..2 |
| `ProviderTransportFailure` | `ProviderFailure` | 0 | 0 | 0..2 | 0 | 0 | 1..2 |
| `BothModesNotFound` | `NotFound` | 0 | 0 | 0 | 0 | 0 | 0 |
| `EmptyAddress` | `InvalidInput` | 0 | 0 | 0 | 0 | 0 | 0 |

- `RepresentativeLocation`은 `DistrictEdge`, `MultipleDistrictContainment`, `OutsideBoundaryCoverage`에만 존재한다. 후보가 둘이면 해당 reason을 실제로 일으킨 candidate를 대표로 사용한다. 둘 다 같은 reason을 일으킨 경우에만 Road를 deterministic tie-break로 사용하고 `selectionBasis`에 기록한다.
...
- generic provider reason에서 후보 2개를 허용하는 경우는 Road와 Parcel 주소 해석이 모두 성공한 뒤 단일 `Boundary` WFS reported/schema/transport failure가 발생한 경우뿐이다. 주소 mode failure가 하나라도 있으면 성공 후보는 최대 1개다.
- Road/Parcel이 서로 다른 종류의 provider failure를 반환해도 둘 다 `Road → Parcel` 순서로 보존한다. public `ReasonCode`는 첫 ordered failure의 canonical code가 가리키는 reason이다. generic failure는 mode 중복을 허용하지 않고 각 failure가 `PROVIDER_REPORTED_ERROR`, `PROVIDER_SCHEMA_INVALID`, `PROVIDER_TRANSPORT_FAILURE` 중 자신의 typed code를 가져야 한다. Boundary failure는 address mode failure와 섞지 않는다.
...
- 실행 우선순위는 empty input → 두 address mode 결과 수집 → address provider failure → both not found → dual EPSG:5179 거리 `>150m` → bootstrap field schema failure → dual cohort bootstrap disagreement → boundary provider failure와 completeness → candidate별 edge → multiple → outside → 모든 unique strict의 legal-name mismatch → 두 unique 법정동 disagreement → success 순이다. 호출할 필요가 없는 boundary provider의 잠재적 실패는 앞세우지 않으며, 먼저 결정된 결과로 실행을 중단한다.
...
- 주소 request cache는 resolver 전용 typed DTO만 저장한다. successful candidate는 30일, `NotFound`는 24시간 캐시하고 `ForceRefresh`면 우회한다. provider reported/schema/transport failure와 malformed 응답은 저장하지 않는다. `CapturedAtUtc`는 provider 응답을 처음 취득한 시각으로 cache hit에서도 유지하고, cache delivery 시각은 별도 `DeliveredAtUtc`로 기록한다. schema/version mismatch·손상 entry는 삭제하고 network로 재조회한다.
- cohort boundary cache는 exact-set 검사가 끝난 complete result만 30일 저장한다. incomplete/duplicate/pagination/schema/transport failure는 저장하지 않으며 `ForceRefresh` 정책은 주소 cache와 같다.
...
- resolver 전용 boundary DTO는 raw feature에서 WFS `emd_cd`, `full_nm`, `emd_kor_nm`, topology를 함께 투영한다. 기존 `LegalDistrictRecord`/`BoundaryRecord`의 name 하나로 축약된 projection은 주소 corroboration에 재사용하지 않는다.
