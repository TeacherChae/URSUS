...
- pagination과 예상 서울 지역 coverage가 완결된 경우에만 정상 결과로 사용
...
- 서울 통계는 서울 법정동에만 사용한다. 지원 범위 밖 요청은 API 호출 전에 차단한다.
- mapping되지 않은 지역을 전체 평균으로 채우지 않고 missing/mapping quality로 남긴다.
